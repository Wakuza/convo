...
<?php
    ...
    * See every employees
    */
    $queryAll = "SELECT e.employeeID, e.firstname, e.lastname, e.position_name, CONCAT(s.firstname, ' ', s.lastname) AS supervisor, e.hire_date, e.review_date, e.employment_status FROM employee s RIGHT JOIN employee e ON e.supervisorID = s.employeeID";
    $resultAll = mysqli_query($link, $queryAll);

    $num_rows = mysqli_affected_rows($link);

    echo "<table id='example' class='display' cellspacing='0' width='1010px'>";
    if($user_data["supervisorID"] == null) {
